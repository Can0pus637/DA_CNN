...
module tb_TOP;
  // Parameters (must match DUT)
    ...
    localparam int K_tile                = 5;
    localparam int N_tile                = 1;
    localparam int CHUNK_M               = 1;
    localparam int CHUNK_K               = 5;
  ...
  always_ff @(posedge clk or posedge rst) begin
    ...
    print_enable <= 0;
      ...
      //  if(uut.rst_Net==0) begin//uut.u_LetNet.u_layerX.u_GEMM.t0==1&&
      if (!rst 
      &&  //(uut.u_LetNet.u_layerX.t==0)&&
     // &&uut.u_LetNet.u_layerX.rd_k==0
     // &&uut.u_LetNet.u_layerX.rd_n==0
     // &&uut.u_LetNet.u_layerX.rd_mi==0
     // &&
     (
     //uut.u_LetNet.u_layerX.Layer==4||
     uut.u_LetNet.u_layerX.LAYER_done==1)
      ...
      $display("im2_flattenA_addr = %0d", uut.u_LetNet.u_layerX.im2_flattenA_Adder);
      $display("im2_flattenB_addr = %0d", uut.u_LetNet.u_layerX.im2_flattenB_Adder);

      //dump_kernel1(uut.u_LetNet.u_layerX.Kernel_input1);
  
      //print_layer_ram2_layer2();dump_kernel2(uut.u_LetNet.u_layerX.Kernel_input2);
   
      //print_layer_ram1_layer3();//dump_kernel3(uut.u_LetNet.u_layerX.Kernel_input3);
    
      //print_layer_ram2_layer4();
      //dump_kernel4(uut.u_LetNet.u_layerX.Kernel_input4);
     
      //print_layer_ram1_layer5();dump_kernel5(uut.u_LetNet.u_layerX.Kernel_input5);
     
     // print_layer_ram2_layer6();dump_kernel6(uut.u_LetNet.u_layerX.Kernel_input1);
     
      //print_LAYER_Result();
      
      $display("time[%0t] ", $time);
      $display("layer_ram1[Adder_Layer1] = %0d", uut.u_LetNet.u_layerX.layer_ram1[uut.u_LetNet.u_layerX.Adder_Layer1]);
      
      //$display("row_data_kernel1 = %b", uut.row_data_kernel1);
      //$display("row_data_map     = %b", uut.row_data_map);
      //$display("addr             = %0d", uut.addr);
      //$display("rst_Net          = %0d", uut.rst_Net);
      $display("old_data = %0d", uut.u_LetNet.u_layerX.old_data);
      $display("col_padding = %0d", uut.u_LetNet.u_layerX.col_padding);
      $display("row_padding = %0d", uut.u_LetNet.u_layerX.row_padding);
      $display("Adder_Layer1 = %0d", uut.u_LetNet.u_layerX.Adder_Layer1);

      $display("Layer=%0d  M=%0d N=%0d K=%0d,out = %0d ",
                uut.u_LetNet.Layer, uut.u_LetNet.M, uut.u_LetNet.N, uut.u_LetNet.K,out);
      
      $display("LAERY.t = %0d,cal_k = %0d cal_m=%0d cal_n=%0d  ",
               uut.u_LetNet.u_layerX.t, uut.u_LetNet.u_layerX.cal_k, uut.u_LetNet.u_layerX.cal_m,uut.u_LetNet.u_layerX.cal_n);
      $display("LAERY.t = %0d,aim_k = %0d aim_m=%0d aim_n=%0d  ",
               uut.u_LetNet.u_layerX.t, uut.u_LetNet.u_layerX.aim_k, uut.u_LetNet.u_layerX.aim_m,uut.u_LetNet.u_layerX.aim_n);  
      $display("C_out = %0d",
               uut.u_LetNet.u_layerX.u_Array_Input.C_out[0]);
      $display("row_padding = %0d,col_padding = %0d",
               uut.u_LetNet.u_layerX.row_padding,uut.u_LetNet.u_layerX.col_padding);         
               
      $display("ArrayInput t=%0d ", uut.u_LetNet.u_layerX.u_Array_Input.t);
      $display("im2_cnt = %0d im2_done = %0d", uut.u_LetNet.u_layerX.im2_cnt,uut.u_LetNet.u_layerX.im2_done);
      $display("C_out", uut.u_LetNet.u_layerX.u_Array_Input.C_out[0]);
     
      print_flattenA();
      print_flattenB();
      print_GEMM_Result();
      $display("layer4_sum0=%d layer4_sum1=%d layer4_sum2=%d layer4_sum3=%d", uut.u_LetNet.u_layerX.layer4_sum[0],uut.u_LetNet.u_layerX.layer4_sum[1],uut.u_LetNet.u_layerX.layer4_sum[2],uut.u_LetNet.u_layerX.layer4_sum[3]);
      
    //  print_flattenA_reg();
    //  print_flattenB_reg();Layer
      $display("im2_flattenA_Adder", uut.u_LetNet.u_layerX.im2_flattenA_Adder);
      $display("im2_flattenB_Adder", uut.u_LetNet.u_layerX.im2_flattenB_Adder);
      if(uut.u_LetNet.u_layerX.Layer==0) begin
            print_layer_ram2_start();print_layer_ram1_layer1();
      end
      if(uut.u_LetNet.u_layerX.Layer==1) begin
            print_layer_ram2_layer2();
      end
      if(uut.u_LetNet.u_layerX.Layer==2) begin
            print_layer_ram1_layer3();
      end
      if(uut.u_LetNet.u_layerX.Layer==3) begin
            print_layer_ram2_layer4();
      end
      $display("STRIDE", uut.u_LetNet.u_layerX.STRIDE);
      $display("LAERY.t = %0d,rd_k = %0d rd_mi=%0d rd_mj=%0d rd_n=%0d LAYER_done=%0d LAYER_rd_done=%0d LAYER_cal_done=%0d LAYER_aim_done=%0d  ",
               uut.u_LetNet.u_layerX.t, uut.u_LetNet.u_layerX.rd_k, uut.u_LetNet.u_layerX.rd_mi, uut.u_LetNet.u_layerX.rd_mj,uut.u_LetNet.u_layerX.rd_n,
               uut.u_LetNet.u_layerX.LAYER_done, uut.u_LetNet.u_layerX.LAYER_rd_done, uut.u_LetNet.u_layerX.LAYER_cal_done, uut.u_LetNet.u_layerX.LAYER_aim_done);
      $display("v_idx = %0d", uut.u_LetNet.u_layerX.u_im2.v_idx);
      $display("u_idx = %0d", uut.u_LetNet.u_layerX.u_im2.u_idx);
      $display("c_idx = %0d", uut.u_LetNet.u_layerX.u_im2.c_idx);
      $display("--------------------------------------------------");
      end
    end
  end
...
//    for (ch = 0; ch < uut.u_LetNet.Y3; ch++) begin
//      $display("map_input_ch%0d = [", ch);
//      for (r = 0; r < uut.u_LetNet.Y2; r++) begin
//        for (c = 0; c < uut.u_LetNet.Y1; c++) begin
//          idx = ch * (uut.u_LetNet.Y1 * uut.u_LetNet.Y2)
//              + r  *  uut.u_LetNet.Y1
//              + c;
//          $write("%0d", uut.u_LetNet.layer_input_A[idx]);
//          if (c != uut.u_LetNet.Y1-1)
//            $write(" ");
//        end
//        if (r != uut.u_LetNet.Y2-1)
//          $write(";\n");
//        else
//          $write("\n");
//      end
      ...
      for (r = 0; r < 32; r++) begin
        for (c = 0; c < 32; c++) begin
          idx = ch * (32 * 32)
              + r  *  32
              + c;
          $write("%0d", uut.u_LetNet.u_layerX.layer_ram2[idx]);
          if (c != 32)
            $write(" ");
        end
        if (r != 32-1)
      ...
      for (r = 0; r < 29; r++) begin
        for (c = 0; c < 29; c++) begin
          idx = ch * (29 * 29)
              + r  *  29
              + c;
          $write("%0d", uut.u_LetNet.u_layerX.layer_ram1[idx]);
          if (c != 29)
            $write(" ");
        end
        if (r != 29-1)
...
task print_flattenA;
  ...
  for (mm = 0; mm < CHUNK_M; mm = mm + 1) begin
    ...
    for (kk = 0; kk < CHUNK_K; kk = kk + 1) begin
      $write("%0d", uut.u_LetNet.u_layerX.flattenA[mm][kk]);
      if (kk < CHUNK_K-1)
        $write(", ");
    end
    // 行末加分号或换行
...
task print_flattenA_reg;
  ...
  for (mm = 0; mm < CHUNK_M; mm = mm + 1) begin
    ...
    $write("  ");
    for (kk = 0; kk < CHUNK_K; kk = kk + 1) begin
      $write("%0d", uut.u_LetNet.u_layerX.flattenA_reg[mm][kk]);
      if (kk < CHUNK_K-1)
        $write(", ");
    end
    // 行末加分号或换行
...
task print_flattenB;
  ...
  for (kk = 0; kk < CHUNK_K; kk = kk + 1) begin
    ...
    for (nn = 0; nn < CHUNK_N; nn = nn + 1) begin
      $write("%0d", uut.u_LetNet.u_layerX.flattenB[kk][nn]);
      if (nn < CHUNK_N-1)
        $write(", ");
    end
    if (kk < CHUNK_K-1)
...
task print_flattenB_reg;
  ...
  for (kk = 0; kk < CHUNK_K; kk = kk + 1) begin
    $write("  ");
    for (nn = 0; nn < CHUNK_N; nn = nn + 1) begin
      $write("%0d", uut.u_LetNet.u_layerX.flattenB_reg[kk][nn]);
      if (nn < CHUNK_N-1)
        $write(", ");
    end
    if (kk < CHUNK_K-1)
...
endtask
  ...
  task automatic print_LAYER_Result;
    integer ch, i, idx;
    begin
      $display("--- LAYER_Result 按通道输出: 共 %0d 通道 × 每通道 %0d 值 ---", uut.u_LetNet.N, uut.u_LetNet.M);
      for (ch = 0; ch < uut.u_LetNet.N; ch = ch + 1) begin
        $write("通道%0d:", ch);
        for (i = 0; i < uut.u_LetNet.M; i = i + 1) begin
          // 原始是一维数组，行主序存储：第 i 行第 ch 列
          idx = i  + ch* uut.u_LetNet.M;
          $write(" %0d", uut.u_LetNet.u_layerX.layer_ram1[idx]);
        end
        $write("\n");
      end
 ...
 task automatic print_C_mat;
    ...
    begin
      ...
      for (i = 0; i < N_tile; i = i + 1) begin
          ...
          $write(" %0d ", uut.u_LetNet.u_layerX.GEMM_Result[i]);
  ...
  task print_GEMM_Result;
    ...
    $display("--- GEMM_Result [%0dx%0d] ---", CHUNK_M, CHUNK_N);
      ...
      for (nn = 0; nn <CHUNK_N ; nn++)
        $write("\n %0d\n", uut.u_LetNet.u_layerX.GEMM_Result[nn]);
